...
public class DailySummaryWindow extends javax.swing.JFrame {
    ...
    public DailySummaryWindow() {
        ...
        tableHeader.setFont(new Font("Arial", Font.BOLD, 12)); // Change font and size
        tableHeader.setPreferredSize(new Dimension(tableHeader.getPreferredSize().width, 30)); // Optional: Increase header height
    }
    ...
    try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)) {
        ...
        // If no sales exist for today
        if (!hasSales) {
            model.addRow(new Object[]{"No sales recorded for today", "", ""});
        }

    } catch (SQLException e) {
        // Log the error and show a generic error message in the table
    ...
    try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD)) {
        ...
        // If no sales exist in the inventory
        if (!hasSales) {
